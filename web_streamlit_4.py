...
>>>>>>> ee413e42764bcd3202bc9ddbb433f54010d9b7f5
    ...
    # Option to upload a CSV file
        ...
        st.markdown(
            '<p style="color: blue; font-weight: bold; ">OPTION 2: Upload a file</p>',
            unsafe_allow_html=True,
        )

        uploaded_file = st.file_uploader(
            "Upload CSV file (The file must contain a column with author: authors, title: title of the note and text: news)",
            type="csv",
        )
    if uploaded_file:
        ...
        if missing_columns:
            st.warning(
                f"The uploaded file is missing the following required columns: {', '.join(missing_columns)}. Please add them and try again."
            )
    ...
    if st.button("Evaluate", key="evaluate_button"):
        if (
            "author" in test_df.columns
            and "title" in test_df.columns
            and "text" in test_df.columns
        ):
            # Procesar datos
    ...
    if st.button("Evaluate", key="evaluate_button"):
        if (
            ...
            if unigram_vectors_without_stopwords.shape[1] < 100:
                ...
                st.warning(
                    f"The document does not have enough unique words. It has {unigram_vectors_without_stopwords.shape[1]} more than the 100 it should have"
                )
            else:
                # Hacer predicciones
    ...
    if st.button("Save Data"):
        ...
        if os.path.exists(save_path):
            ...
            existing_data = pd.DataFrame(
                columns=["id_new", "title", "author", "text", "label_predict"]
            )
            st.dataframe(existing_data)
        ...
        if not duplicates.empty:
            ...
            if st.button("Confirm Save Duplicates"):
                if not st.session_state["duplicate_entries"].empty:
                    combined_data = (
                        pd.concat([existing_data, test_df])
                        .drop_duplicates()
                ...
                if not st.session_state["duplicate_entries"].empty:
                    ...
                    st.session_state["duplicate_entries"] = (
                        pd.DataFrame()
                    )  # Clear duplicate entries after saving
                else:
